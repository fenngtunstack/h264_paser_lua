    ...
    local function export_h264_to_file()
        ...
        local function real_write_to_file(stream_info, str_bytes, begin_with_nalu_hdr)
            if first_run then
                ...
                if begin_with_nalu_hdr then
                    -- save SPS or PPS
                    -- bit32 local nalu_type = bit.band(str_bytes:byte(0,1), 0x1F)
					local nalu_type = str_bytes:byte(0,1) & 0x1F
        ...
        local function sort_and_write(stream_info, frame)
            ...
            for i = 1, #frame.packs - 1, 1 do
                ...
                -- bit32 if bit.band(seq1+1, 0xFFFF) ~= seq2 then
        ...
        local function write_to_file(stream_info, str_bytes, begin_with_nalu_hdr, timestamp, seq, end_of_nalu)
            if drop_uncompleted_frame and not first_run then -- sort and drop uncompleted frame
                ...
                if p then  -- add this pack to frame
                    if begin_with_nalu_hdr then
                        -- bit32 p.nalu_type = bit.band(str_bytes:byte(1), 0x1F)
						p.nalu_type = str_bytes:byte(1) & 0x1F
                    end
                ...
                if begin_with_nalu_hdr then
                    -- bit32 frame.nalu_type = bit.band(str_bytes:byte(1), 0x1F)
					frame.nalu_type = str_bytes:byte(1) & 0x1F
                end
        ...
        local function process_fu_a(stream_info, h264, timestamp, seq)
            ...
            -- bit32 local end_of_nalu =  (bit.band(fu_hdr, 0x40) ~= 0)
			local end_of_nalu = ((fu_hdr & 0x40) ~= 0)
            -- bit32 if bit.band(fu_hdr, 0x80) ~= 0 then
			if (fu_hdr & 0x80) ~= 0 then
                -- start bit is set then save nalu header and body
                -- bit32 local nalu_hdr = bit.bor(bit.band(fu_idr, 0xE0), bit.band(fu_hdr, 0x1F))
				local nalu_hdr = (fu_idr & 0xE0) | (fu_hdr & 0x1F)
        ...
        function my_h264_tap.packet(pinfo,tvb)
            ...
            for i,h264_f in ipairs(h264s) do
                ...
                -- bit32 local hdr_type = bit.band(h264:get_index(0), 0x1F)
				local hdr_type = (h264:get_index(0) & 0x1F)
